<?php
	try {
		...
		if($resultado->rowCount() > 0) {
			$row = $resultado->fetch();
			if ($row['senha'] == $senha){
				setcookie("msg", "Logado com sucesso !", time() + 1,"/alunoProfessor/visao");
				session_start();
				$_SESSION['usuarioNome'] = $row['nome'];
				header('Location: ../visao/indexAluno.php');
			}else{
				setcookie("msg", "Senha incorreta", time() + 1,"/alunoProfessor/visao");
		...
		setcookie("msg", "Erro: ". $e->getMessage(), time() + 1,"/alunoProfessor/visao");
		header('Location: ../visao/index.php');
	}
?>